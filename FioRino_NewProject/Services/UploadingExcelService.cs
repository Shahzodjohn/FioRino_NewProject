            ...
            try
            {
                ...
                using (var stream = new MemoryStream())
                {
                    ...
                    using (var package = new ExcelPackage(stream))
                    {
                        ...
                        for (int row = 10; row < rowcount - 1; row += 3)
                        {
                            ...
                            while (isClassicCategory && tempColumn < 19)
                            {
                                ...
                                if (cell != null)
                                {
                                    ...
                                    var GtinPaging = worksheet.Cells[row, 21].Value == null ? "" : worksheet.Cells[row, 21].Value.ToString().Trim();
                            ...
                            if (!isClassicCategory)
                            {
                                ...
                                while (isFasterCategory && tempColumn < 19)
                                {
                                    ...
                                    if (cell != null)
                                    {
                                        ...
                                        tempColumn++;
                                        var productName = worksheet.Cells[row, 2].Value.ToString().Trim();
                                        var GtinPage = worksheet.Cells[row + 1, 21].Value == null ? "" : worksheet.Cells[row + 1, 21].Value.ToString().Trim();
                            ...
                            if (!isFasterCategory)
                            {
                                var isBrakCategory = worksheet.Cells[row + 2, indexColumn].Value == null ? false :
                                                   worksheet.Cells[row + 2, indexColumn].Value.ToString().ToLower() == "brak" ? true : false;
                                category = await _categoryRepository.FindCategoryById(38);
                                tempColumn = indexColumn + 2;
                                while (isBrakCategory && tempColumn < 20)
                                {
                                    var cell = worksheet.Cells[row + 2, tempColumn].Value;
                                    if (cell != null)
                                    {
                                        Int32.TryParse(cell.ToString(), out productAmount);
                                        sizeName = (worksheet.Cells[9, tempColumn].Value == null ? "" : worksheet.Cells[9, tempColumn].Value.ToString());
                                        ...
                                        tempColumn++;
                                        var productName = worksheet.Cells[row, 2].Value.ToString().Trim();
                                        var GtinPage = worksheet.Cells[row + 2, 21].Value == null ? "" : worksheet.Cells[row + 2, 21].Value.ToString().Trim();
                                        var findStan = await _OrderRepository.FindStorage(GtinPage);
                                        var findProduct = await _OrderRepository.FindDmProduct(GtinPage);
                                        if (findProduct == null)
                                        {
                                        ...
                                        var SkuNumber = worksheet.Cells[row, 1].Value.ToString().Trim();
                                        var SkuId = await _OrderRepository.InsertSKUcodes(SkuNumber);
                                        var CheckingProductValidation = await _OrderRepository.CheckingProductValidation(productName, category.Id, SizeId);
                                        var productNameworksheet = worksheet.Cells[row, 2].Value.ToString().Trim();
                                        var uniquproductFind = await _OrderRepository.CreateUniqueProduct(productNameworksheet);
                                        var ProductId = 0;
                                        if (CheckingProductValidation == null && GtinPage != "")
                                    ...
                                    if (cell == null)
                                        tempColumn++;
                                    if (tempColumn == 20 && cell == null)
                                        isBrakCategory = false;
                                }
                            }
